...
const makeFolder = async () => {
    ...
    await fs.access(folderPath, permissionVal, (err) => {
        if (err.code !== 'ENOENT') {
            throw err;
        }
        console.log('폴더없음');
    ...
    await fs.open(filePath, 'w', (err, fd) => {
        if (err) {
            throw err;
        }
    ...
    // await fs.rename('./folder3/fileName.js', './folder3/newfile.js', (err) => {
    //     if (err) {
    //         throw err;
    //     }
    //     console.log('파일 이름 바꾸기 성공');
    ...
    await fs.rename(filePath, renameFileName , (err)=>{
        if(err){
            throw err;
        }
